...
WHY THIS EXISTS. The docs moved from `/azure-emulators/<slug>/` to
`/azure-emulators/docs/<slug>/`, which is the shape the rest of the
family uses. That moved **4 published routes**, and a moved URL is a 404 for
every link already pointing at it: this project's own README, anything in the
sibling repositories, and anything outside them nobody can enumerate. All four
are 404 on the live site today, and `/04-chain-test/` is still linked from
committed documentation in this family, measured. The links nobody can grep are
...
`/azure-emulators/docs/01-quickstart/` and nothing answers the
root-level path anyone actually linked to. The stubs must be written outside
Astro, which is here.

THIS SITE PUBLISHES NO BADGE ENDPOINTS of its own, so nothing else is written
into the output directory. This script clears it before it writes.
"""
...
# project's 4 published routes keeps its own name under /docs/. The one route
