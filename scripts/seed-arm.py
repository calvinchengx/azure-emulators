#!/usr/bin/env python3
"""Give the family stack a vault resource and one role assignment.

ARM governs the vault by default, and ARM's rule is Azure's: no assignment
means no access. Without a seed, `docker compose up` would stand up a vault
that refuses its own quickstart -- more configuration to get started, not
less. So the stack seeds exactly what Azure gives you when you create a vault
in the portal: the resource, and a grant for the principal that created it.

The principal is entra's seeded daemon app, which is also the identity
e2e/chain authenticates as, so the chain's data-plane probe reads 404 (the
secret does not exist) rather than 403 (denied). That test asserts the
difference, which is what keeps this file honest.

Everything here is reachable by hand; the file is the documentation for it.
Opt out with KV_ARM_URL= (see docker-compose.yml).

Adapted from azure-keyvault-emulator's scripts/seed-arm.py, which seeds that
repo's own stack the same way. Compose mounts the script from the repo it runs
in, so the family stack carries its own copy.
